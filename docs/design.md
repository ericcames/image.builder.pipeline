...
## 11. SNO Installer Kit — Agent-Based Installer ISO

Phase 5 ([#86](https://github.com/ericcames/image.builder.pipeline/issues/86)).
A fundamentally different artifact from the guest OS images in §9–§10: this
builds the **OpenShift platform itself**, not a VM that runs on top of it. The
"OS-only" constraint (§9.3, §10.7) governs guest images; an ABI ISO is a
platform installer and carries operator subscriptions by design.

### 11.1 What gets built

An Agent-Based Installer ISO generated by `openshift-install agent create image`.
Bootable on bare metal (UEFI), fully unattended, deploys:

1. Single Node OpenShift (control-plane + worker on one node)
2. AAP 2.7 operator (`aap-operator`, channel `stable-2.7`)
3. OpenShift Virtualization / CNV operator (`kubevirt-hyperconverged`)
4. Compliance Operator
5. CIS L1 node hardening (`ocp4-cis-node` MachineConfigs)

OCP version is configurable, defaulting to the latest stable channel. Monthly
rebuilds track z-stream updates.

### 11.2 Pull secret constraint

The ABI ISO embeds the OpenShift pull secret (from `console.redhat.com`) in
its Ignition config. A pull secret is a credential and **cannot** be in a
public image.
...
Generated at ISO build time from the kit contents plus user-provided secrets:

```
workdir/
  install-config.yaml       # SNO: controlPlane.replicas=1, compute[0].replicas=0
  agent-config.yaml         # bare metal: NIC, disk, static IP or DHCP
  openshift/                # Day 0 extra manifests (numbered for ordering)
...
**Profile:** `ocp4-cis-node` (CIS Red Hat Enterprise Linux CoreOS) from the
Compliance Operator's bundled content.

**Day 0 — MachineConfigs in the ISO.** Pre-rendered from known Compliance
Operator remediations. Applied during bootstrap before the cluster is fully up.
Covers: kernel sysctl params, audit rules, file permissions, mount options,
SSH hardening.
...
2. Install Compliance Operator, create `ScanSettingBinding` for `ocp4-cis-node`
3. Wait for scan, extract `ComplianceRemediation` objects → MachineConfigs
4. Save as static manifests in `playbooks/files/sno-manifests/cis-l1/`
...
different IP after a reboot, breaking those records and making the cluster
